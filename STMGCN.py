...
class ST_MGCN(nn.Module):
    def __init__(self, M:int, seq_len: int, n_nodes: int, input_dim: int,
                 lstm_hidden_dim: int, lstm_num_layers: int, sta_kernel_config:dict,
                 gconv_use_bias: bool, gconv_activation=nn.ReLU):
        super().__init__()
        self.M = M
        ...
        for m in range(self.M):
            ...
            gcn = GCN(K=self.sta_K, input_dim=lstm_hidden_dim, hidden_dim=lstm_hidden_dim,
                      bias=gconv_use_bias, activation=gconv_activation)
            self.gcn_list.append(gcn)
        self.fc = nn.Linear(in_features=lstm_hidden_dim, out_features=input_dim, bias=True)     # fusion: add
